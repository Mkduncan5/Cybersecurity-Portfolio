Cybersecurity Portfolio 🔐
👋 About Me
I am a cybersecurity enthusiast passionate about ethical hacking, penetration testing, and security analysis. This portfolio showcases my projects, research, and skills in cybersecurity.

📂 Projects
1️⃣ Social Engineering Attack Simulation
Tools Used: SET Toolkit, DNSChef, Kali Linux
Description: Demonstrated a credential harvesting attack using cloned login pages.
2️⃣ Network Penetration Testing
Tools Used: Metasploit, Nmap, Wireshark
Description: Performed vulnerability scanning and exploited a system using Metasploit.
3️⃣ Log Analysis & SIEM Alerts
Tools Used: Splunk, ELK Stack
Description: Created detection rules for brute-force login attempts and suspicious network activity.
📖 Skills
✅ Penetration Testing
✅ Incident Response
✅ Log Analysis (SIEM)
✅ Social Engineering
✅ Cryptography
📬 Contact
📧 Email: mkdun27@gmail.com
🔗 LinkedIn: [Your LinkedIn URL]
📂 GitHub: [Your GitHub Profile URL]# Cybersecurity-Portfolio
## Certifications

- **Cisco Certified Network Associate (CCNA)**
  - **Date Obtained**: January 2024
  - **Verification Link**: [View Certification](https://www.credly.com/badges/89827af1-a1a3-4072-a85f-3870d0deb8bb/public_url)
## Certifications

- **Cisco Certified Network Associate (CCNA)**
  - **Date Obtained**: January 2024
  - **Verification Link**: [View Certification](https://www.credly.com/badges/89827af1-a1a3-4072-a85f-3870d0deb8bb/public_url)

- **Cisco CyberOps Associate**
  - **Date Obtained**: August 2024
  - **Verification Link**: [View Certification](https://www.credly.com/badges/c8137472-aa96-41f6-a611-e259079aeac8/public_url)

- **Cisco Certified DevNet Associate**
  - **Date Obtained**: September 2024
  - **Verification Link**: [View Certification](https://www.credly.com/badges/ee591068-78d1-473d-a55a-795ca4597edc/public_url)
